...
public class SrsPublisher {
   ...
   // private SrsCameraView mCameraView;
            ...
            public void run() {
                ...
                while (!Thread.interrupted()) {
                    if (sendVideoOnly) {
                       ...
                       // Log.d("buffer size", ""+mPcmBuffer.length);
                        int size = mic.read(mPcmBuffer, 0, mPcmBuffer.length);
                       // Log.d("mic size", ""+mPcmBuffer.length);
                        if (size > 0) {
    ...
    public int getCameraId() {
        return 0;// mCameraView.getCameraId();
    }
    
    public Camera getCamera() {
        return null;// mCameraView.getCamera();
    }     
    ...
    public boolean switchCameraFilter(MagicFilterType type) {
        return false;//mCameraView.setFilter(type);
    }
